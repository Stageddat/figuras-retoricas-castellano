...
type Pregunta = [string, string]; // [nombre, definicion]
...
function DefinicionQuiz() {
    ...
    useEffect(() => {
        ...
        const preguntasMezcladas = Object.entries(figuras)
            .map(([nombre, datos]) => [nombre, datos.Definicion] as Pregunta)
            .sort(() => Math.random() - 0.5);

        setPreguntas(preguntasMezcladas);
        if (preguntasMezcladas.length > 0) {
            genOpciones(preguntasMezcladas[0][1]);
        }
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, []);

    // gen random answers
    const genOpciones = (respuestaCorrecta: string) => {
        const figuras = loadfigurasRetoricasJSON();
        const todasOpciones = Object.values(figuras).map(f => f.Definicion);
    ...
    const manejarRespuesta = (definicionSeleccionada: string) => {
        const [nombreFigura, respuestaCorrecta] = preguntas[preguntaActual];

        setRespuestas(prev => [
            ...prev,
            {
                pregunta: nombreFigura,
                respuestaCorrecta,
                respuestaUsuario: definicionSeleccionada
            }
        ]);

        if (definicionSeleccionada === respuestaCorrecta) {
            setPuntuacion(puntuacion + 1);
        }

        if (preguntaActual < preguntas.length - 1) {
            setPreguntaActual(preguntaActual + 1);
            genOpciones(preguntas[preguntaActual + 1][1]);
        } else {
            ...
            <div className="max-w-4xl mx-auto">
                ...
                <div className="bg-[#0a0a0a] border-2 border-[#272727] p-6 rounded-lg mb-8">
                    <div className="text-2xl text-gray-200 leading-relaxed">
                        ¿Cuál es la definición de "<span className="font-bold">{preguntas[preguntaActual][0]}</span>"?
                    </div>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                    {opciones.map((definicion) => (
                        <button
                            key={definicion}
                            onClick={() => manejarRespuesta(definicion)}
                            className="bg-[#0a0a0a] border-2 border-[#272727] text-gray-100 text-lg py-4 px-6 rounded-lg transition duration-200 hover:border-gray-600 hover:scale-105 text-left"
                        >
                            {definicion}
                        </button>
                    ))}
